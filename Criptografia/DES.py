...
# Permutação das tabelas e sboxes
IP = (
...
def encrypt(msg, key, decrypt=False):
    ...
    # aplicando a função no esquema de feistel
    L_last = L0
    R_last = R0
    for i in range(1,17):
        if decrypt: # usando as chaves na ordem inversa
            i = 17-i
        L_round = R_last
...
def generate_round_keys(C0, D0):
    # retorna um dicionario de 16 keys (um por rodada)
    ...
    # criação de 16 pares de chaves diferentes
    ...
    # round_keys[1] para primeira rodada
    #           [16] para 16º rodada
    del round_keys[0]
 
    # formam as chaves para concatenar com CiDi 1<=i<=16 e aplicar em PC2
    for i, (Ci, Di) in round_keys.items():
        Ki = (Ci << 28) + Di
        round_keys[i] = permutation_by_table(Ki, 56, PC2) # 56bits -> 48bits
 
    return round_keys
 
k = 0x0e329232ea6d0d73 # 64 bits
k2 = 0x133457799BBCDFF1
m = 0x8787878787878787
m2 = 0x0123456789ABCDEF
 
def prove(key, msg):
    print('chave:       {:x}'.format(key))
    print('mensagem:   {:x}'.format(msg))
    cipher_text = encrypt(msg, key)
    print('cifrado: {:x}'.format(cipher_text))
    plain_text = encrypt(cipher_text, key, decrypt=True)
    print('decifrado: {:x}'.format(plain_text))
